...
{
	public class SqlServerExtractor : DbServerExtractorBase, IDbServerExtractor
	{
		#region Fields
		...
		public SqlServerExtractor(ExtractorConfig config) : base(config)
		{
		}
		...
		private string GetQuery(ExtractorConfig config)
		{
			...
			if (condition.WhereClause != null)
			{
				...
				query.Replace("{StartDate}", start.ToString(AppConfig.SqlServerQueryFormat));
				query.Replace("{EndDate}", end.ToString(AppConfig.SqlServerQueryFormat));
			}
