...
{
	public class MySQLExtractor : DbServerExtractorBase, IDbServerExtractor
	{
		#region Fields
		...
		public MySQLExtractor(ExtractorConfig config) : base(config)
		{
		}
		...
		public int Extract()
		{
			...
			using (var conn = new MySqlConnection(_config.DbConn.ConnectionString))
			{
				try
				{
					conn.Open();
					...
					using (var command = new MySqlCommand(query, conn))
					{
						...
						_logger.Info(@"[{0}][{1}][{2}]", "Mariasql/MySQL ServerExtractor", _config.GetFileName(), command.CommandText);
		...
		private string GetQuery(ExtractorConfig config)
		{
			...
			if (condition.WhereClause != null)
			{
				...
				query.Replace("{StartDate}", start.ToString(AppConfig.MySQLFormat));
				query.Replace("{EndDate}", end.ToString(AppConfig.MySQLFormat));
			}
