...
{
	public class SqlServerV8Extractor : DbServerExtractorBase, IDbServerExtractor
	{
		#region Fields
		...
		public SqlServerV8Extractor(ExtractorConfig config) : base(config)
		{
		}
		...
		public int Extract()
		{
			...
			using (var conn = new SqlConnection(_config.DbConn.ConnectionString))
			{
				try
				{
					conn.Open();
					...
					using (var command = new SqlCommand(query, conn))
					{
						...
						_logger.Info(@"[{0}][{1}][{2}]", "SqlServerExtractor", _config.GetFileName(), command.CommandText);
		...
		private string GetQuery(ExtractorConfig config)
		{
			...
			if (condition.WhereClause != null)
			{
				...
				query.Replace("{StartDate}", start.ToString(AppConfig.SqlServerV8QueryFormat));
				query.Replace("{EndDate}", end.ToString(AppConfig.SqlServerV8QueryFormat));
			}
