...
{
	public class SqlServerV8ConfigBuilder : IDbServerConfigBuilder
	{
		#region Fields
		...
		public SqlServerV8ConfigBuilder()
		{
		}
		...
		private List<Column> GetColumns(SqlConnection conn, out string query, out SqlCommand cmd, SqlServerTableInfo table)
		{
				...
				while (reader.Read())
				{
					try
					{
						columns.Add(new Column()
						{
							...
							DataType = DbDataTypeRepository.Instance.GetType(DbServerKind.SqlServerV8, reader["data_type"].ToString()),
		...
		{
			using (var conn = new SqlConnection(dbConnInfo.ConnectionString))
			{
				try
				{
					conn.Open();
					...
					var query = "SELECT table_catalog, table_schema, table_name FROM INFORMATION_SCHEMA.TABLES;";
					...
					// Get Tables
					var tables = new List<SqlServerTableInfo>();
					using (var reader = cmd.ExecuteReader())
					{
						while (reader.Read())
						{
							tables.Add(new SqlServerTableInfo()
							{
					...
					foreach (var table in tables)
					{
						...
						config.QueryCondition.WhereClause = "{DeltaField} >= convert(datetime,'{Start}', 120) AND {DeltaField} < convert(datetime,'{End}', 120)"; // select convert(datetime,'2008-12-19 17:30:09.123', 120);
		...
		public void Update(ExtractorConfig config)
		{
			...
			using (var conn = new SqlConnection(dbConnInfo.ConnectionString))
			{
				try
				{
					...
					var query = $"SELECT MIN({config.DeltaField}) AS init_start FROM {config.QueryCondition.FromClause};";
					var cmd = new SqlCommand
					{
						Connection = conn,
						CommandType = CommandType.Text,
					...
					if (config.DeltaFieldType.Equals(DeltaFieldType.None.ToString()))
					{
						...
						config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerV8DeltaTypeConverter[DeltaFieldType.None];
					}
					else
					{
						using (var reader = cmd.ExecuteReader())
						{
							while (reader.Read())
							{
								...
								if (config.DeltaFieldType.Equals(DeltaFieldType.DateTime.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerV8DeltaTypeConverter[DeltaFieldType.DateTime];
								}
								else if (config.DeltaFieldType.Equals(DeltaFieldType.String8.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerV8DeltaTypeConverter[DeltaFieldType.String8];
								}
								else if (config.DeltaFieldType.Equals(DeltaFieldType.String14.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerV8DeltaTypeConverter[DeltaFieldType.String14];
								}
