...
{
	public class SqlServerConfigBuilder : IDbServerConfigBuilder
	{
		#region Fields
				...
				while (reader.Read())
				{
					try
					{
						columns.Add(new Column()
						{
							...
							DataType = DbDataTypeRepository.Instance.GetType(DbServerKind.SqlServer, reader["data_type"].ToString()),
			...
			using (var conn = new SqlConnection(dbConnInfo.ConnectionString))
			{
				try
				{
					...
					foreach (var table in tables)
					{
						...
						config.QueryCondition.WhereClause = "{DeltaField} >= convert(datetime2,'{StartDate}', 120) AND {DeltaField} < convert(datetime2,'{EndDate}', 120)"; // select convert(datetime2,'2008-12-19 17:30:09.1234567', 120);
						...
						using (var reader = cmd.ExecuteReader())
						{
							while (reader.Read())
							{
								...
								if (config.DeltaFieldType.Equals(DeltaFieldType.DateTime.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerDeltaTypeConverter[DeltaFieldType.DateTime];
								}
								else if (config.DeltaFieldType.Equals(DeltaFieldType.String8.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerDeltaTypeConverter[DeltaFieldType.String8];
								}
								else if (config.DeltaFieldType.Equals(DeltaFieldType.String14.ToString()))
								{
									...
									config.QueryCondition.WhereClause = DbDataTypeRepository.Instance.SqlServerDeltaTypeConverter[DeltaFieldType.String14];
								}
							}
						}
					}
