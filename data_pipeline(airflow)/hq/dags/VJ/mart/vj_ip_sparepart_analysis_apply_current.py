...
# ─────────────────────────────────────────────────────────────
with DAG(
    dag_id="vj_ip_sparepart_analysis_apply_current",
    default_args=default_args,
    description="VJ IP SparePart This Month,Year Analysis",
    ...
    analysis_tables = [
        ("analysis_zone_machine_year", current_year),
        ("analysis_zone_year", current_year),
        ("analysis_zone_machine_month", current_month),
        ("analysis_zone_month", current_month),
    ]

    cleangroup = []
    for table_name, target_date in analysis_tables:
        clean_task = PythonOperator(
            task_id=f"clean_{table_name}_current_data",
            python_callable=pg_v2.clean_table_with_condition, 
            op_args=[SCHEMA_NAME, table_name, "date", target_date],
    ...
    for group_by in ["zone", "zone_machine"]:
        tg_year = create_analysis_taskgroup(dag, group_by, current_year)
        taskgroups.append(tg_year)

        tg_month = create_analysis_taskgroup(dag, group_by, current_month)
        taskgroups.append(tg_month)
