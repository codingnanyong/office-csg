...
SQL_QUERIES = {
    "employee": "SELECT user_id, user_name FROM pcc.pcc_mst_user WHERE factory = 'DS';",
    "location": "SELECT process_loc, loc_name,step_code,level1,level2 FROM rtls.tb_process_loc ORDER BY process_loc;",
    "opcd": "SELECT op_cd, op_name,op_local_name FROM pcc.pcc_mst_opcd WHERE factory = 'DS' ORDER BY sort_no;",
    "common": "SELECT com_div, com_cd, com_name FROM pcc.pcc_mst_com WHERE factory = 'DS' AND com_div = 'SEASON' ORDER BY sort_no;",
}
...
# ────────────────────────────────────────────────────────────────────────────
with DAG(
    dag_id="hq_master_update",
    default_args=default_args,
    description="HQ Master",
    schedule="@daily",
