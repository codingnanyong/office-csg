"""
OS Banbury HMI Maintenance Backfill DAG
========================================
유지보수 데이터를 시간별로 수집하는 Backfill DAG
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="os_banb_hmi_maintenance_backfill",
    default_args=DEFAULT_ARGS,
    schedule_interval=None,
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=["JJ", "OS", "Banbury", "HMI", "maintenance", "backfill", "bronze layer"],
):
    for eq in EQUIPMENTS:
        PythonOperator(
            task_id=f"os_banb_hmi_maintenance_data_backfill_eq{eq['equipment_id']}",
        ...
        PythonOperator(
            task_id=f"os_banb_hmi_maintenance_state_backfill_eq{eq['equipment_id']}",
