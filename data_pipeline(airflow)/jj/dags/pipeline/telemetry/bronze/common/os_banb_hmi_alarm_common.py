"""
OS Banbury HMI Alarm Common Functions
======================================
공통 함수 및 설정을 모아둔 모듈
"""
...
EQUIPMENTS = [
    {
        ...
        "var_key_data": "last_extract_time_os_banb_hmi_alarm_data_eq1",
        "var_key_stat": "last_extract_time_os_banb_hmi_alarm_state_eq1",
    },
    {
        "equipment_id": "3",
        "equipment_value": 3003,
        "conn_id": "maria_jj_os_banb_3",
        "var_key_data": "last_extract_time_os_banb_hmi_alarm_data_eq3",
        "var_key_stat": "last_extract_time_os_banb_hmi_alarm_state_eq3",
    },
]

SCHEMA_NAME = "bronze"
TABLE_NAME_DATA = "os_banb_hmi_alarm_data"
TABLE_NAME_STAT = "os_banb_hmi_alarm_stat"
...
def build_extract_sql_data(start_dt: datetime, end_dt: datetime, equipment_value: int) -> str:
    """Build SQL query for alarm data extraction"""
    ...
    return f"""
        SELECT 
            ...
            COALESCE(d.ResultDate, d.CreateDate) AS result_ts
        FROM rtf_alarm_data d
...
def build_extract_sql_stat(start_dt: datetime, end_dt: datetime, equipment_value: int) -> str:
    """Build SQL query for alarm stat extraction"""
    start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
    end_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")
    return f"""
        SELECT 
            ...
            COALESCE(st.ResultDate, st.CreateDate) AS result_ts
        FROM rtf_alarm_state st
...
def run_incremental_data(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Run incremental collection for alarm data"""
    ...
    while cur_start <= hard_end:
        ...
        logging.info(f"🚀 Alarm data incremental 쿼리 (Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            batch_size = 2000
            batch_total = 0
            
            for batch_rows in mysql.execute_query_streaming(sql, "os_banb_hmi_alarm_data_incremental_extract", batch_size=batch_size):
                if batch_rows:
                    insert_data = prepare_rows(batch_rows, datetime.utcnow())
                    columns = [
                        "factory", "equipment", "seq_no", "pid", "alarm", "level", "ucl", "uwl", "cl", "lwl", "lcl",
                        "usl", "lsl", "p_value", "sensor_group", "active", "period", "user_id",
                        "result", "result_date", "create_date", "result_ts", "etl_extract_time", "etl_ingest_time"
                    ]
                    conflict_columns = ["factory", "equipment", "seq_no", "result_ts"]
            ...
            if batch_total > 0:
                logging.info(f"📦 os_banb_hmi_alarm_data 추출 row 수 (Eq{equipment_id}): {batch_total:,}")
    ...
    logging.info(f"✅ os_banb_hmi_alarm_data incremental 완료 (Eq{equipment_id}), 총 {total_rows} rows")
...
def run_incremental_stat(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Run incremental collection for alarm stat"""
    # hard_end 계산 (Skip된 경우 Variable 업데이트용)
    hard_end = (datetime.now(INDO_TZ) - timedelta(hours=HOURS_OFFSET_FOR_INCREMENTAL)).astimezone(INDO_TZ)
    hard_end = _eoh(hard_end)
    ...
    while cur_start <= hard_end:
        ...
        logging.info(f"🚀 Alarm state incremental 쿼리 (Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            batch_size = 2000
            batch_total = 0
            
            for batch_rows in mysql.execute_query_streaming(sql, "os_banb_hmi_alarm_stat_incremental_extract", batch_size=batch_size):
                if batch_rows:
                    insert_data = prepare_rows(batch_rows, datetime.utcnow())
                    columns = [
                        "factory", "equipment", "pid", "alarm", "level", "ucl", "uwl", "cl", "lwl", "lcl",
                        "usl", "lsl", "p_value", "sensor_group", "active", "period", "user_id",
                        "result", "result_date", "create_date", "result_ts", "etl_extract_time", "etl_ingest_time"
                    ]
                    conflict_columns = ["factory", "equipment", "pid", "result_ts"]
            ...
            if batch_total > 0:
                logging.info(f"📦 os_banb_hmi_alarm_stat 추출 row 수 (Eq{equipment_id}): {batch_total:,}")
    ...
    logging.info(f"✅ os_banb_hmi_alarm_stat incremental 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}
...
def process_backfill_data(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Process backfill for alarm data"""
    try:
        mysql = MySQLHelper(conn_id=conn_id)
        pg = PostgresHelper(conn_id=TARGET_POSTGRES_CONN_ID)
        if not check_mysql_connection_quick(mysql, conn_id, timeout_seconds=5):
    ...
    while cur_start <= hard_end:
        cur_end = min(_eoh(cur_start), hard_end)
        sql = build_extract_sql_data(cur_start, cur_end, equipment_value)
        logging.info(f"🚀 Alarm backfill 쿼리(시간단위, Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            rows = mysql.execute_query(sql, "os_banb_hmi_alarm_data_backfill_extract", None) or []
            logging.info(f"📦 os_banb_hmi_alarm_data 추출 row 수 (Eq{equipment_id}): {len(rows)}")
    ...
    logging.info(f"✅ os_banb_hmi_alarm_data backfill 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}


def process_backfill_stat(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Process backfill for alarm stat"""
    try:
        mysql = MySQLHelper(conn_id=conn_id)
        pg = PostgresHelper(conn_id=TARGET_POSTGRES_CONN_ID)
        if not check_mysql_connection_quick(mysql, conn_id, timeout_seconds=5):
    ...
    while cur_start <= hard_end:
        cur_end = min(_eoh(cur_start), hard_end)
        sql = build_extract_sql_stat(cur_start, cur_end, equipment_value)
        logging.info(f"🚀 Alarm state backfill 쿼리(시간단위, Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            rows = mysql.execute_query(sql, "os_banb_hmi_alarm_stat_backfill_extract", None) or []
            logging.info(f"📦 os_banb_hmi_alarm_stat 추출 row 수 (Eq{equipment_id}): {len(rows)}")
    ...
    logging.info(f"✅ os_banb_hmi_alarm_stat backfill 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}
