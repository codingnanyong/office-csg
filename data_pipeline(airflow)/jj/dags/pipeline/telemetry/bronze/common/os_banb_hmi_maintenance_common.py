"""
OS Banbury HMI Maintenance Common Functions
===========================================
공통 함수 및 설정을 모아둔 모듈
"""
...
EQUIPMENTS = [
    {
        ...
        "var_key_data": "last_extract_time_os_banb_hmi_maintenance_data_eq1",
        "var_key_stat": "last_extract_time_os_banb_hmi_maintenance_state_eq1",
    },
    {
        "equipment_id": "3",
        "equipment_value": 3003,
        "conn_id": "maria_jj_os_banb_3",
        "var_key_data": "last_extract_time_os_banb_hmi_maintenance_data_eq3",
        "var_key_stat": "last_extract_time_os_banb_hmi_maintenance_state_eq3",
    },
]

SCHEMA_NAME = "bronze"
TABLE_NAME_DATA = "os_banb_hmi_maintenance_data"
TABLE_NAME_STAT = "os_banb_hmi_maintenance_state"
...
def build_extract_sql_data(start_dt: datetime, end_dt: datetime, equipment_value: int) -> str:
    """Build SQL query for maintenance data extraction"""
    start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
    end_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")
    return f"""
        SELECT 
            1 AS factory,
            {equipment_value} AS equipment,
            d.SeqNo AS seq_no,
            d.Maintenance AS maintenance,
            d.Level AS level,
            d.Cycle AS cycle,
            d.Min AS min_value,
            d.Max AS max_value,
            ...
            d.User AS user_id,
            d.Result AS result,
            d.MinDate AS min_date,
            d.MaxDate AS max_date,
            d.BaseDate AS base_date,
            d.ResultDate AS result_date,
            d.CreateDate AS create_date
        FROM rtf_maintenance_data d
        WHERE COALESCE(d.ResultDate, d.CreateDate) >= '{start_str}' AND COALESCE(d.ResultDate, d.CreateDate) <= '{end_str}'
        ORDER BY COALESCE(d.ResultDate, d.CreateDate)
    """


def build_extract_sql_stat(start_dt: datetime, end_dt: datetime, equipment_value: int) -> str:
    """Build SQL query for maintenance stat extraction"""
    start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
    end_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")
    return f"""
        SELECT 
            ...
            st.User AS user_id,
            st.Result AS result,
            st.MinDate AS min_date,
            st.MaxDate AS max_date,
            st.BaseDate AS base_date,
            st.ResultDate AS result_date,
            st.CreateDate AS create_date
        FROM rtf_maintenance_state st
        WHERE COALESCE(st.ResultDate, st.CreateDate) >= '{start_str}' AND COALESCE(st.ResultDate, st.CreateDate) <= '{end_str}'
        ORDER BY COALESCE(st.ResultDate, st.CreateDate)
    """
...
def run_incremental_data(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Run incremental collection for maintenance data"""
    ...
    while cur_start <= hard_end:
        ...
        logging.info(f"🚀 Maintenance data incremental 쿼리 (Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            batch_size = 2000
            batch_total = 0
            
            for batch_rows in mysql.execute_query_streaming(sql, "os_banb_hmi_maintenance_data_incremental_extract", batch_size=batch_size):
                if batch_rows:
                    insert_data = prepare_rows(batch_rows, datetime.utcnow())
                    columns = [
                        "factory", "equipment", "seq_no", "maintenance", "level", "cycle", "min_value", "max_value",
                        "active", "days", "user_id", "result", "min_date", "max_date", "base_date",
                        "result_date", "create_date", "etl_extract_time", "etl_ingest_time"
                    ]
            ...
            if batch_total > 0:
                logging.info(f"📦 os_banb_hmi_maintenance_data 추출 row 수 (Eq{equipment_id}): {batch_total:,}")
    ...
    logging.info(f"✅ os_banb_hmi_maintenance_data incremental 완료 (Eq{equipment_id}), 총 {total_rows} rows")
...
def run_incremental_stat(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Run incremental collection for maintenance stat"""
    # hard_end 계산 (Skip된 경우 Variable 업데이트용)
    hard_end = (datetime.now(INDO_TZ) - timedelta(hours=HOURS_OFFSET_FOR_INCREMENTAL)).astimezone(INDO_TZ)
    hard_end = _eoh(hard_end)
    ...
    while cur_start <= hard_end:
        ...
        logging.info(f"🚀 Maintenance state incremental 쿼리 (Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            batch_size = 2000
            batch_total = 0
            
            for batch_rows in mysql.execute_query_streaming(sql, "os_banb_hmi_maintenance_state_incremental_extract", batch_size=batch_size):
                if batch_rows:
                    insert_data = prepare_rows(batch_rows, datetime.utcnow())
                    columns = [
                        "factory", "equipment", "maintenance", "level", "cycle", "min_value", "max_value",
                        "active", "days", "user_id", "result", "min_date", "max_date", "base_date",
                        "result_date", "create_date", "etl_extract_time", "etl_ingest_time"
                    ]
                    conflict_columns = ["factory", "equipment", "maintenance"]
            ...
            if batch_total > 0:
                logging.info(f"📦 os_banb_hmi_maintenance_state 추출 row 수 (Eq{equipment_id}): {batch_total:,}")
    ...
    logging.info(f"✅ os_banb_hmi_maintenance_state incremental 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}
...
def process_backfill_data(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Process backfill for maintenance data"""
    try:
        mysql = MySQLHelper(conn_id=conn_id)
        pg = PostgresHelper(conn_id=TARGET_POSTGRES_CONN_ID)
        if not check_mysql_connection_quick(mysql, conn_id, timeout_seconds=5):
    ...
    while cur_start <= hard_end:
        cur_end = min(_eoh(cur_start), hard_end)
        sql = build_extract_sql_data(cur_start, cur_end, equipment_value)
        logging.info(f"🚀 Maintenance backfill 쿼리(시간단위, Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            rows = mysql.execute_query(sql, "os_banb_hmi_maintenance_data_backfill_extract", None) or []
            logging.info(f"📦 os_banb_hmi_maintenance_data 추출 row 수 (Eq{equipment_id}): {len(rows)}")
    ...
    logging.info(f"✅ os_banb_hmi_maintenance_data backfill 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}


def process_backfill_stat(equipment_id: str, conn_id: str, var_key: str, equipment_value: int, **context):
    """Process backfill for maintenance stat"""
    try:
        mysql = MySQLHelper(conn_id=conn_id)
        pg = PostgresHelper(conn_id=TARGET_POSTGRES_CONN_ID)
        if not check_mysql_connection_quick(mysql, conn_id, timeout_seconds=5):
    ...
    while cur_start <= hard_end:
        cur_end = min(_eoh(cur_start), hard_end)
        sql = build_extract_sql_stat(cur_start, cur_end, equipment_value)
        logging.info(f"🚀 Maintenance state backfill 쿼리(시간단위, Eq{equipment_id}): {cur_start} ~ {cur_end}\n{sql}")

        try:
            rows = mysql.execute_query(sql, "os_banb_hmi_maintenance_state_backfill_extract", None) or []
            logging.info(f"📦 os_banb_hmi_maintenance_stat 추출 row 수 (Eq{equipment_id}): {len(rows)}")
    ...
    logging.info(f"✅ os_banb_hmi_maintenance_state backfill 완료 (Eq{equipment_id}), 총 {total_rows} rows")
    return {"status": "success", "rows": total_rows}
