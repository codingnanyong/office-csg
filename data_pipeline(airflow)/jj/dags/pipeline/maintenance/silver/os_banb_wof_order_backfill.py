"""
OS BANB Work Order Silver Layer Backfill DAG (Bronze → Silver)
==============================================================
Bronze 레이어에서 Silver 레이어로 특정 설비의 Work Order 데이터를 전처리하여 초기 적재

Source: bronze.wof_order_raw
Target: silver.os_banb_wof_order
...
from dags.pipeline.maintenance.silver.common.os_banb_wof_order_common import (
...
INCREMENT_KEY = "last_extract_time_os_banb_wof_order"
...
def backfill_silver_etl_task(**kwargs) -> dict:
    ...
    logging.info("="*80)
    logging.info("🚀 OS BANB Work Order Silver Backfill 시작")
    ...
    if extracted_count > 0:
        ...
        prepared_data = prepare_insert_data(data, ingest_time)
        loaded_count = load_data_to_silver(postgres, prepared_data)
    
    # Set initial variable for incremental DAG (전일 23:59:59)
    end_time = (datetime.now(INDO_TZ) - timedelta(days=2)).replace(hour=23, minute=59, second=59, microsecond=0)
...
# ────────────────────────────────────────────────────────────────
dag = DAG(
    'os_banb_wof_order_backfill',
    default_args=DEFAULT_ARGS,
    description='OS BANB Work Order Silver Layer Backfill (Bronze → Silver)',
    schedule_interval=None,  # Manual trigger only
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['JJ', 'maintenance', 'silver layer', 'backfill', 'work_order', 'OS', 'Banbury']
)
