"""
OS BANB Work Order Silver Layer Incremental ETL DAG (Bronze → Silver)
=====================================================================
Bronze 레이어에서 Silver 레이어로 특정 설비의 Work Order 데이터를 전처리하여 적재
...
# ────────────────────────────────────────────────────────────────
dag = DAG(
    'os_banb_wof_order_incremental',
    default_args=DEFAULT_ARGS,
    description='OS BANB Work Order Silver Layer Incremental ETL (Bronze → Silver)',
    schedule_interval= None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['JJ', 'maintenance', 'silver layer', 'incremental', 'work_order', 'OS', 'Banbury']
)
