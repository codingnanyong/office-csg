"""ANS Target Raw Incremental DAG (Oracle → Bronze)"""
import logging
from datetime import datetime, timedelta, timezone
from airflow import DAG
from airflow.operators.python import PythonOperator
from airflow.models import Variable
from dags.pipeline.maintenance.bronze.common.ans_target_raw_common import (
    parse_datetime,
    extract_data,
    load_data,
    update_variable,
...
INCREMENT_KEY = "last_extract_time_ans_target_raw"
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="ans_target_raw_incremental",
    default_args=DEFAULT_ARGS,
    schedule_interval="@monthly",  # 월별 실행
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=["JJ", "raw", "bronze layer", "incremental", "maintenance"]
) as dag:
