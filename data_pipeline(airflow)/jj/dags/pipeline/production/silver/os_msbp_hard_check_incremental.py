...
from dags.pipeline.production.silver.common.os_msbp_hard_check_common import (
    parse_datetime,
    extract_silver_data,
    load_silver_data,
    update_variable,
...
def daily_silver_incremental_collection_task(**kwargs) -> dict:
    ...
    logging.info(f"📅 Silver 데이터 수집 시작: {start_str} ~ {end_str}")
    logging.info(f"📊 처리 날짜: {start_date.strftime('%Y-%m-%d')}")
    logging.info(f"🔍 필터 조건: LAB_COMP_CD = 'OSR'")
    ...
    if row_count > 0:
        ...
        logging.info(f"✅ Silver 데이터 수집 완료: {row_count} rows")
        ...
        logging.info(f"⚠️ 수집할 Silver 데이터가 없습니다: {start_str} ~ {end_str}")
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="os_msbp_hard_check_incremental",
