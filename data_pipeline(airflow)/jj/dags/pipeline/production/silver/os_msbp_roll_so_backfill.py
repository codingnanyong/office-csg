...
def process_silver_monthly_batch(
    ...
    logging.info(f"🔄 Silver SO 루프 {loop_count}/{expected_loops} 시작")
    start_str = start_date.strftime("%Y-%m-%d %H:%M:%S")
    end_str = end_date.strftime("%Y-%m-%d %H:%M:%S")
    logging.info(f"Silver SO 배치 처리 중: {start_str} ~ {end_str}")
    data, row_count = extract_silver_data(pg, start_str, end_str)
    if row_count > 0:
        extract_time = datetime.utcnow()
        load_silver_data(pg, data, extract_time)
        logging.info(f"✅ Silver SO 배치 완료: {start_str} ~ {end_str} ({row_count} rows)")
    else:
        logging.info(f"Silver SO 배치에 데이터 없음: {start_str} ~ {end_str}")
...
def backfill_silver_monthly_batch_task(**kwargs) -> dict:
    ...
    logging.info(f"Silver SO Backfill 시작: {start_date} ~ {end_date}")
    ...
    logging.info(f"🎉 Silver SO Backfill 완료! 총 {loop_count}회 루프, {total_processed}개 rows 수집")
    if results:
        logging.info(f"처리 기간: {results[0]['start']} ~ {results[-1]['end']}")

    return {
        "status": "silver_so_backfill_completed",
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="os_msbp_roll_so_backfill",
