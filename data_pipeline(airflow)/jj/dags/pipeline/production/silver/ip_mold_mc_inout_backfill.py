...
INCREMENT_KEY = "last_extract_time_silver_ip_mold_mc_inout"

# ────────────────────────────────────────────────────────────────
# 2️⃣ Main Backfill Logic
# ────────────────────────────────────────────────────────────────
def process_monthly_silver_batch(
    pg: PostgresHelper, 
    start_date: datetime, 
    end_date: datetime,
    loop_count: int,
    ...
    """Process a single monthly Silver batch"""
    ...
    if row_count > 0:
        ...
        load_silver_data(pg, data, extract_time, silver_schema=SILVER_SCHEMA, silver_table_name=SILVER_TABLE_NAME, remove_duplicates=False)
...
def backfill_silver_monthly_batch_task(**kwargs) -> dict:
    """Main Silver backfill task for monthly batch processing"""
    ...
    while current_date < end_date:
        ...
        # Process batch
        batch_result = process_monthly_silver_batch(
            pg, current_date, month_end, loop_count, expected_loops
        )
...
# ────────────────────────────────────────────────────────────────
with DAG(
    dag_id="ip_mold_mc_inout_silver_backfill",
    default_args=DEFAULT_ARGS,
    schedule_interval="@once",
    start_date=datetime(2025, 1, 1),
    catchup=False,
    tags=["CKP","IP","clean", "silver layer", "backfill", "production", "monthly","IP", "mold"]
) as dag:
