"""공통 함수 모듈 - MSPQ IN OSND BT Raw Bronze"""
import logging
...
SCHEMA_NAME = "bronze"
TABLE_NAME = "mspq_in_osnd_bt_raw"
INDO_TZ = timezone(timedelta(hours=7))
INITIAL_START_DATE = datetime(2019, 1, 2, 0, 0, 0)
...
def build_extract_sql(start_date: str, end_date: str) -> str:
    ...
    return f'''
        SELECT
            ...
            MOLD_SUB_DESC, MACHINE_CD, MOLD_ID
        FROM MSPQ_IN_OSND_BT
        WHERE (
    ...
    if data and isinstance(data[0], dict):
        ...
        return [
            (
                ...
                row['MOLD_SUB_DESC'], row['MACHINE_CD'], row['MOLD_ID'],
                extract_time
            ) for row in data
        ]
    else:
...
def load_data(
    ...
    conflict_columns = ["plant_cd", "osnd_id"]
